...
class index extends Component {
    ...
    componentWillMount() {
        callApi('admin/news', 'GET')
            .then(res => {
                const listNews = res.data;
                console.log(listNews.news)

                this.setState({ news: listNews.newss, loading: false })
    ...
    render() {
        ...
        const renderlistNews = news.map((item, index) => {
            return (
                <Table.Row>
                    ...
                    <Table.Cell>{item.picture}</Table.Cell>
